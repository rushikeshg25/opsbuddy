...
// Hook for recent downtime incidents
export const useRecentDowntime = (productId: number, days: number = 30) => {
  // Stabilize dates to prevent constant re-fetching
  // Round to the nearest minute to avoid millisecond changes
  const now = new Date();
  ...
  return useQuery({
    queryKey: [...downtimeKeys.lists(), { productId, days, startDate: startDate.slice(0, 16) }], // Only use date/hour/minute
    queryFn: () => apiClient.getDowntime({
